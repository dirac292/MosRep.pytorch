...
def get_standard_transform(args):
    transform = transforms.Compose([
        transforms.RandomResizedCrop(args.global_size, scale=args.global_scale),
...
def get_local_transform(args):
    transform = transforms.Compose([
        transforms.RandomResizedCrop(args.local_size, scale=args.local_scale),
        transforms.RandomApply(
            [transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)],  # not strengthened
            p=0.8,
        ),
...
class CropsTransform:
    ...
    def __call__(self, sample):
        # Check if 'label' is present in the sample
        # print(f"Sample keys: {sample.keys()}")

        # if 'label' in sample:
        #     # For labeled data (4 keys expected)
        #     _, _, x, label = sample.values()
        # else:
        #     # For unlabeled data (3 keys expected)
        #     _, _, x = sample.values()
        ...
        if self.multi_crop:
            # Query mini crops
            crops_q_local = torch.stack([
                self.transform_local(x) for _ in range(4)
            ], 0)
            
            return crops_q, crops_q_local, crops_k
        else:
            return crops_q, crops_k

    def __repr__(self):
        return self.__class__.__name__ + "(\n" + \
            f"\ttransform_standard={self.transform_standard}\n" + \
            f"\ttransform_local={self.transform_local}\n" + \
            f"\tmulti_crop={self.query_mini_transform}\n" + ")"


class LinearTransform:
    def __init__(self, transform_linear):
        self.transform_linear = transform_linear
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(\n" + \
            f"\ttransform_linear={self.transform_linear}\n)"
